...
def fuck_addresses(addresses):
    ...
    for address in addresses:
        if 'location_name' in address['address'] and len(address['address']['location_name']) > 0:
            head,sep,tail = address['address']['location_name'].partition(' ')

            if tail=='':
                address['address']['location_name'] = ' '.join(['fucking',head]).title()
            else:
                address['address']['location_name'] = ' '.join([head,'fucking',tail]).title()

        else:
            address['address']['location_name'] = 'Some Fucking Building'.upper()
