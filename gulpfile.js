...
gulp.task('styles', function(done) {
    // move over fonts
    gulp.src(config.SRC_DIR + 'fonts/**.*').pipe(gulp.dest(config.DEST_DIR + 'fonts'));

    //move over vendor scripts
    gulp.src(config.SRC_DIR + 'scripts/vendor/**.*').pipe(gulp.dest(config.DEST_DIR + 'scripts/vendor'));
    done();
});

gulp.task('sass', function(done) {
    return gulp
        .src(config.SRC_DIR + 'scss/**/*.scss')
...
gulp.task('images', function(done) {
    gulp.src(config.SRC_DIR + 'images/**').pipe(gulp.dest(config.DEST_DIR + 'images'));
    done();
});
...
gulp.task('watch', gulp.series('images', 'styles', 'sass', 'scripts', 'browser-sync'), function() {
    gulp.watch(config.SRC_DIR + 'scss/**/*.scss', ['sass']); // scss watch for style change
    return buildScript('main.js', true); // browserify watch for JS changes
});

// run 'scripts' task first, then watch for future changes
gulp.task('default', gulp.series('images', 'styles', 'sass', 'scripts'), function() {
